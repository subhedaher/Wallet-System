    ...
    <div class="card">
        <div class="card-body flex flex-col p-6">
            <header class="flex mb-5 items-center border-b border-slate-100 dark:border-slate-700 pb-5 -mx-6 px-6">
                <div class="flex-1">
                    <div class="card-title text-slate-900 dark:text-white">Edit Payment Category</div>
                </div>
            </header>
                ...
                <form class="space-y-4" id="form-data">
                    ...
                    <div class="input-area">
                        <div class="filePreview">
                            ...
                            <div id="file-preview">
                                <img src="{{ Storage::url($paymentCategory->image) }}" alt="">
                            </div>
                        </div>
                    </div>

                    <input type="button" class="btn inline-flex justify-center btn-dark"
                        onclick="editPaymentCategory('{{ route('payment-categories.update', $paymentCategory->id) }}')"
                        value="Save">
                </form>
            </div>
        </div>
...
@section('scripts')
    <script>
        function editPaymentCategory(route) {
            ...
            axios.post(route, formData)
                .then(function(response) {
                    ...
                    window.location.href = '{{ route('payment-categories.index') }}';
